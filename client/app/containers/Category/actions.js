...
export const addCategory = () => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.post(`http://localhost:3000/api/category/add`, newCategory);
...
export const updateCategory = () => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.put(`http://localhost:3000/api/category/${category._id}`, {
...
export const activateCategory = (id, value) => {
  return async (dispatch, getState) => {
    try {
      const response = await axios.put(`http://localhost:3000/api/category/${id}/active`, {
        category: {
          isActive: value
        }
      });
